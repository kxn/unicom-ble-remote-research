...
| 控制指令面 | 已梳理：按键报表、FB `01`/`00` 语音启停、FD02 通知，及厂商库逆向出的 ListenAI 鉴权协议（主机侧验真，链路可忽略），见 [指令面](docs/unicom-ble-commands.md) |

范围仅限这只样机。不能凭“联通/移动/电信遥控器”外观推广为通用协议；芯片厂家仍未确认，音频编码已由下述解码验证。
...
已集成原生 ICO 实时解码：构建后以 `--record-voice` 启动探针，按住语音键录音、松手自动保存 WAV。见 [完整操作与验证](docs/live-voice.md)。
